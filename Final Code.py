...
behavior_proxy = ALProxy("ALBehaviorManager", robotIP, 9559)
ball_color = ''
...
class SpeechEventModule():
    ...
    def __init__(self):
        ...
        vocabulary = ["pick green", "pick blue", "pick red"]
        # Get the service ALMemory.
        self.memory = ses.service("ALMemory")
        # Connect the event callback.
        self.speech_recognition = ses.service("ALSpeechRecognition")
        self.speech_recognition.unsubscribe("subscribeSpeechRecog")
        ...
        self.speech_recognition.subscribe("subscribeSpeechRecog")
        self.first = True

    def on_word_recorgnized(self,value):
        global ball_color
        print "on_word_recorgnized Event detected!"
        print('Word recognized:'+ str(value[0])+' confidence: '+str(value[1]))
        word_detected = value[0]
        if(word_detected==""):
            tts.say('you said'+value[0])
            
            #====
        elif(word_detected=="pick red" and value[1] > 0.45):
            tts.say('Okay! I will be looking for red ball')
            ball_color = "red"
            return
            #=====
        elif (word_detected=='pick green' and value[1] > 0.50):
            tts.say('Okay! I will be looking for green ball')
            ball_color = "green"
            return
             #====
        elif (word_detected=='pick blue' and value[1] > 0.50):
            tts.say('Okay! I will be looking for blue ball')
            ball_color = "blue"
            return
             #====     
...
def take_pics(angleScan, CameraIndex):
    ...
    motionAngles.append(commandAngles)
    print str(commandAngles)
    centers = analyze_img2(ball_color)
...
def CenterOfMassUp(image, ball_color):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    if ball_color == 'red':
        lowera = np.array([170, 115, 0])
        ...
        lowera = np.array([49, 201, 33])
        uppera = np.array([128, 255, 255])
        lowerb = np.array([49, 201, 33])
        upperb = np.array([128, 255, 255])
    elif ball_color == 'green':
        lowera = np.array([22, 47, 12])
        uppera = np.array([85, 255, 255])
        lowerb = np.array([22, 47, 12])
        upperb = np.array([85, 255, 255])
    else:                                   #default case is for red ball
    ...
    if len(cont)>=1:
        ...
        for l in range(len(cont)):
            ...
            for k in range(len(contour)):
                ...
                err=err+[e]
            ERR=np.mean(err)
            if ERR < minE and radius >= 8 and radius < 60:
                minim = l
                minE = ERR
                Radius = radius
                Center = icenter
...
def CenterOfMassDown(image, ball_color):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    kernel = np.ones((5,5),np.uint8)
    if ball_color == 'red':
        lowera = np.array([170, 115, 50])
...
def analyze_img(ball_color):
    CM = []
    for i in range(0, 7):
        img = cv2.imread(path + "camImage" + str(i) + ".png")
        cm = CenterOfMassUp(img, ball_color)
        CM.append(cm)
    return CM

def analyze_img2(ball_color):
    CM = []
    for i in range(0, 3):
...
def scan_area(_angleSearch, CameraIndex):
    ...
    commandAngles = motion.getAngles(names, useSensors)
    motionAngles.append(commandAngles)
    print 'commandAngles: '+str(commandAngles)
    motion.angleInterpolationWithSpeed("Head", [maxAngleScan/3, 0.035], 0.1)
    ...
    motionAngles.append(commandAngles)
    print str(commandAngles)
    centers = analyze_img(ball_color)
    return [centers, motionAngles]
...
def initial_scan(ball_color):
    print "Entered in initial_scan function"
    state = 0
...
def walkUp(cm, delta):
    ...
    while cm[0] < 420 and cm[0] > 0:
        ...
        if cm[0] == 0 and cm[1] == 0:
            ...
            cm2 = CenterOfMassUp(img, ball_color)
            lowerFlag = 1
            break
        else:
...
def walkDown(cm, delta):
    ...
    while cm[0] >= 0  and cm[0] < 300:
        ...
        pic(im_num, 1)
        img = cv2.imread(im_num)
        cm = CenterOfMassDown(img, ball_color)
        print('TEST')
        print(im_num, cm)
        if cm == [0, 0]:
            print ("DONE")
...
def kickBall():
    ...
    print("Entered in to kickball function")
    isEnabled  = True
...
def main(robotIP, PORT=9559):
    ...
    motion.wakeUp()
    SpeechEventModule().__init__()
    taskCompleteFlag = 0
    names = behavior_proxy.getInstalledBehaviors()
    #print "Behaviors on the robot:"
    #print names
    #
    while taskCompleteFlag == 0:
        print 'inside while taskCompletedFlag'
        print 'ball color'+ str(ball_color)

        if(ball_color == 'red' or ball_color == 'green' or ball_color == 'blue'):
            motion.moveInit()
           
            ballPosition, delta, camIndex = initial_scan(ball_color)
            if camIndex == 0:
                zero_head()
            ...
            #kickBall()
            motion.moveTo(0.15,0,0)        
            behavior_proxy.runBehavior('singlehandani-e08ea4/behavior_1')
            #motion.rest()
